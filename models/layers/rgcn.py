...
from utils.constants import GNN_EDGE_LABELS_KEY, GNN_EDGE_NORM, GNN_MSG_KEY, GNN_NODE_FEAT_IN_KEY


class RGCNLayer(nn.Module):
    def __init__(self, g, node_dim, edge_dim, out_feats, activation=None, dropout=None, bias=None, is_cuda=True):
        """
        ...
        self.weight = nn.Parameter(torch.Tensor(self.num_bases, 1, self.out_feat))
    ...
    def propagate(self, g):
        ...
        def msg_func(edges):
            edges.data[GNN_EDGE_LABELS_KEY] = edges.data[GNN_EDGE_LABELS_KEY].type(torch.cuda.LongTensor if self.is_cuda else torch.LongTensor)
            ...
            print('edges.src[GNN_NODE_FEAT_IN_KEY].size', edges.src[GNN_NODE_FEAT_IN_KEY].size())
            print('edges.src[GNN_NODE_FEAT_IN_KEY].unsqueeze(1).size', edges.src[GNN_NODE_FEAT_IN_KEY].unsqueeze(1).size())
            print('w.size', w.size())
            w = torch.squeeze(w, 2)
            print('w.size', w.size())
